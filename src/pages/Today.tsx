  ...
  return (
    <div className="max-w-content mx-auto px-4 md:px-8 py-6 md:py-8">
      <PageHeader
        kicker="TODAY"
        title={tr('今日学习')}
        description={tr('一个主要知识点、一个快捷键、一个短练习。每天推进一点，长期就是很大的距离。')}
      />

      {/* 今日概览 */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Metric value={todaySessions.length} label={tr('今日学习记录')} tone="on" />
        <Metric value={stats.completed} label={tr('累计完成课时')} />
        <Metric value={stats.reviewCount} label={tr('待复习')} />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card
          ...
          eyebrow={tr('MAIN · 今日主课')}
          title={main?.title ?? tr('暂无课程')}
          ...
          {main && (
            <>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-3">
                <Tag>{tpl('路线 {r}', { r: main.routeId })}</Tag>
                <Tag>{tpl('第 {n} 周', { n: main.week })}</Tag>
        ...
        {/* 每日快捷键 */}
        <Card
          eyebrow={tr('SHORTCUT · 每日一记')}
          title={shortcut ? `${shortcut.keys} · ${shortcut.nameCn}` : tr('暂无')}
        >
          {shortcut && (
            <>
              ...
              <div className="mt-3 rounded-lg border border-dark-border bg-dark-bg px-3 py-2">
                <p className="text-3xs text-text-tertiary mb-1">{tr('室内设计用法')}</p>
                <p className="text-xs text-text-secondary leading-relaxed">
                  {shortcut.interiorUse}
                </p>
              </div>
              {shortcut.suRef && shortcut.suRef !== tr('—（无）') && (
                <p className="text-3xs text-text-tertiary mt-3">
                  {tpl('对应 SketchUp：{v}', { v: shortcut.suRef })}
                </p>
              )}
              <Link
                to="/shortcuts"
        ...
        <Card
          className="lg:col-span-2"
          eyebrow={tr('REVIEW · 待复习')}
          title={
            reviewItems.length > 0
              ? tpl('{n} 项需要复习', { n: reviewItems.length })
              : tr('暂无待复习内容')
          }
          action={
            reviewItems.length > 0 ? (
        ...
        {/* 路线快捷入口 */}
        <Card eyebrow={tr('ROUTES · 路线')} title={tr('按计划推进')}>
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs font-medium text-text-primary group-hover:text-blender-orange transition-colors">
                        {tpl('路线 {r} · {name}', { r: r.id, name: r.name })}
                      </p>
                      <span className="text-3xs text-text-tertiary">{pct}%</span>
                    </div>
                    <ProgressBar
                      value={pct}
          ...
          <div className="flex items-center gap-2 mt-4 pt-4 border-t border-dark-border">
            ...
            <p className="text-3xs text-text-tertiary">
              {tpl('已坚持 {a} 天，连续 {b} 天', {
                a: stats.totalStudyDays,
                b: stats.consecutiveDays,
              })}
            </p>
          </div>
        </Card>
      ...
      {/* 轻量学习入口 */}
      <div className="mt-6">
        <Card eyebrow={tr('LIGHT MODE · 轻量学习')} title={tr('没有 Blender 的时候也能看')}>
          <p className="text-xs text-text-secondary leading-relaxed">
            {tr('出差、通勤或手边没有电脑时，可以从这里挑一节课，只看讲义部分。')}
          </p>
            ...
            {allLessons
              ...
              .map((l) => (
                <Link
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-dark-border bg-dark-elevated px-3 py-1.5 text-3xs text-text-secondary hover:text-blender-orange hover:border-blender-orange/50 transition-colors"
                >
